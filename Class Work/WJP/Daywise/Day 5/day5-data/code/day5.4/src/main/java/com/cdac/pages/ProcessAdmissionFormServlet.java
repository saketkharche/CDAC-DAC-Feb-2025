...
public class ProcessAdmissionFormServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter pw = response.getWriter()) {
			...
			pw.print("student details from 1st page - "+newStudent+"<bt/>");
		//	pw.flush();//renders resp
			...
			//5. include the clnt to the NEXT page in the SAME request
			rd.include(request, response);
			/*
			 * WC - 
			 * 1. DOES NOT Clear (discard) response buffer(retains)
			 * 2. suspends exec of this method
			 * 3. invokes next page's doPost
			 * 4. next page can add dyn resp - retained
			 * 5. control comes back 
			 * 6. 1st page can continue to add dyn resp 
			 */
			System.out.println("exec control came back....");
			pw.print("<h6>resp after include....</h6>");
		}
