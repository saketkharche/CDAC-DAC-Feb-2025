...
4.1 public static ExecutorService newFixedThreadPool(int maxNoOfThreads): This method returns thread pool executor whose maximum size is fixed.If all n threads are busy performing the task and additional tasks are submitted, then they will have to wait  in the queue until thread is available.
4.2 
